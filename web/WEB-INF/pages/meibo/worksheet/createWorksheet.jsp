...
<div style="font-size: 14px;">
	<h1 style="margin-left: 20px;">
		<c:choose>
			<c:when test="${orderItem.order.isProof eq 0}">产品制作明细</c:when>
			<c:otherwise>样品制作明细</c:otherwise>
		</c:choose>
	</h1>
	<form id="fm_generateWorksheet">
	...
	<table style="border-collapse: collapse; margin-left: 20px; border-width: 0px;" border="1" width="95%">
		<tr>
			<td style="border-width: 0px;" width="50%"><strong>名称：</strong><input type="text"
				value="${orderItem.productName }" style="width: 200px;" name="productName"/></td>
			<td style="border-width: 0px;" width="50%"><strong>编号：</strong><input type="text"
				value="${orderItem.productNo }" style="width: 200px;" name="productNo"/></td>
		</tr>
		<tr>
			<td style="border-width: 0px;"><strong>规格：</strong><input type="text"
				value="${orderItem.contourSize }" style="width: 200px;" name="contourSize"/></td>
			<td style="border-width: 0px;"><strong>数量：</strong><input type="text"
				value="${orderItem.productCount }" style="width: 200px;" name="productCount"/></td>
		</tr>
		<tr>
			<td style="vertical-align: top;"><strong>工程图：</strong>
				<div style="text-align: center;">
					<img style="max-width: 240px; max-height: 240px;" src="${orderItem.projectImage }" />
					<input type="hidden"  name="projectImage" value="${orderItem.projectImage }"/>
				</div>
			</td>
			<td style="vertical-align: top;"><strong>效果图：</strong>
				<div style="text-align: center;">
					<img style="max-width: 240px; max-height: 240px;" src="${orderItem.effectImage }" />
					<input type="hidden"  name="effectImage" value="${orderItem.effectImage }"/>
				</div>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td colspan="2" style="border-width: 0px;"><strong>工艺：</strong></td>
		</tr>
		<tr>
		...
		<tr>
			<td colspan="2" style="border-width: 0px;"><strong>加工时间：</strong></td>
		</tr>
		<tr>
			<td colspan="2" style="border-width: 0px;">
				<input type="text" id="startDate" name="startDate" readonly="readonly" onclick="WdatePicker({startDate:'%y年%M月01日00时',dateFmt:'yyyy年MM月dd日HH时',alwaysUseStartDate:true});" value="<fmt:formatDate value="${orderItem.order.startDate}" pattern="yyyy年MM月dd日00时" />"/>
				至<input type="text" id="endDate"  name="endDate" readonly="readonly" onclick="WdatePicker({startDate:'%y年%M月01日00时',dateFmt:'yyyy年MM月dd日HH时',alwaysUseStartDate:true});" value="<fmt:formatDate value="${orderItem.order.endDate}" pattern="yyyy年MM月dd日00时" />"/>，共计<input type="text" readonly="readonly" id="realUse"/>
			</td>
		</tr>
	</table>
</div>
...
<div id="dlg-buttons" style="text-align: center; margin-top: 20px;">
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok"
		onclick="saveFormData('generateWorksheet',beforeSave,'/meibo/worksheet/saveWorksheet.html',saveSuccess)">保存</a> 
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel"
		onclick="javascript:$('#win_generateWorksheet').window('close')">取消</a>
</div>
<script type="text/javascript">
	$('#realUse').on('click',function(){
		//2013年02月13日21时
	...
	function saveSuccess(data){
		if(data){
			...
			$('#win_generateWorksheet').window('close')
